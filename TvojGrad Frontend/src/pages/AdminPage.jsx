...
export default function AdminPage({ events, approveEvent, rejectEvent, deleteEvent, adminRequests = [], approveAdmin, rejectAdmin }) {
  ...
  return (
    <div className="main">
      <div className="page-header">
        ...
        <div className="page-sub">Upravljanje dogadjajima, promocijama i zahtjevima organizatora</div>
      </div>
            ...
            <table className="admin-table">
              ...
              <tbody>
                {shown.map((e) => (
                  <tr key={e.id}>
                    <td>
                      <strong>{e.coverImg ? "" : e.emoji} {e.title}</strong>
                      {e.status === "na_cekanju_promovisana" && <div style={{ fontSize: 11, color: G.warning, fontWeight: 700 }}>Trazi promociju</div>}
                    </td>
                    ...
                    <td>
                      <div style={{ display: "flex", gap: 4 }}>
                        ...
                        <button className="action-btn action-delete" onClick={() => deleteEvent(e.id)}>Brisi</button>
                      </div>
                    </td>
                  </tr>
                ))}
                {shown.length === 0 && <tr><td colSpan={8} style={{ textAlign: "center", color: G.muted, padding: "2rem" }}>Nema dogadjaja.</td></tr>}
              </tbody>
            </table>
          )}
        </div>
